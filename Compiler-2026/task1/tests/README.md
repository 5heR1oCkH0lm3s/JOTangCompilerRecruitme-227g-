...
`testcases26` 中原有的 `.out` 是程序执行结果，无法证明前端生成了正确 AST。本任务采用两层检查：
...
```bash
# 只跑 10 个用例
python3 scripts/test_frontend.py --max-cases 10

# 只跑路径包含 op_priority 的用例
python3 scripts/test_frontend.py --filter op_priority

# 已构建时跳过构建
python3 scripts/test_frontend.py --skip-build

# 只检查能否生成基本 AST，不比较参考摘要
python3 scripts/test_frontend.py --no-golden

# 暂时跳过非法程序拒绝测试
python3 scripts/test_frontend.py --no-negative
```
