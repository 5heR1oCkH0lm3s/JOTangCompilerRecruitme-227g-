...
1. 请你先阅读task2目录下的项目，分析其CMAKE结构，然后自行构建其整个项目，并请你自行配置`launch.json`和`task.json`文件，从而在vscode可以调试整个项目
...
4. **(拓展，很难)**如果你对高层IR感兴趣，可以基于task2目录下的编译器前中端构建属于你的高层IR，并实现SCF/CF层级优化，比如将while循环转成for循环，在for循环层级上实现LoopFuse优化
